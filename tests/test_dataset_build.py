...
class TestDatasetBuild(unittest.TestCase):
    ...
    def test_build_dataset_multi_horizon(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            features_root = root / "features_root"
            feat_dir = features_root / "features" / "BTCUSDT"
            ...
            rows = []
            for i in range(12):
                rows.append({"ts": i, "mid_price": 100 + i * 0.05, "ret_1": 0.001, "vol_z_20": 0.1, "is_synthetic": False})
            (feat_dir / "part-000.jsonl").write_text("\n".join(__import__('json').dumps(r) for r in rows) + "\n", encoding="utf-8")
            ...
            out = build_dataset(
                ...
                datasets_root=root / "datasets",
                symbols=["BTCUSDT"],
                horizon_steps_list=[3, 6],
            )
            self.assertGreater(out["rows"], 0)
            ds = sorted((root / "datasets").glob("dataset_*/dataset.jsonl"))[-1]
            data = [__import__('json').loads(x) for x in ds.read_text(encoding="utf-8").splitlines()]
            horizons = {int(r["horizon_steps"]) for r in data}
            self.assertEqual(horizons, {3, 6})
